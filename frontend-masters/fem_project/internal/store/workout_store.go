...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	tx, err := pg.db.Begin() // tx stands for transaction
	if err != nil {
		return nil, err
	}
	defer tx.Rollback()
	...
	for _, entry := range workout.Entries {
		query := `
		INSERT INTO workout_entries (workout_id, exercise, sets, reps, duration_seconds, weight, notes, order_index)
...
func (pg *PostgresWorkoutStore) GetWorkoutByID(id int64) (*Workout, error) {
  ...
  `
	...
	entryQuery := `
  SELECT id, exercise, sets, reps, duration_seconds, weight, notes, order_index
...
func (pg *PostgresWorkoutStore) UpdateWorkoutStore(workout *Workout) error {
	tx, err := pg.db.Begin()
	if err != nil {
		return err
	}
